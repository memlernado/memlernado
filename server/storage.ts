...
export interface IStorage {
  ...
  createPasswordResetToken(
    userId: string
  ): Promise<{ token: string; tokenRecord: PasswordResetToken }>;
  ...
  getWorkspaceInvitationByToken(
    token: string
  ): Promise<WorkspaceInvitationWithRelations | undefined>;
  getPendingWorkspaceInvitations(
    workspaceId: string
  ): Promise<WorkspaceInvitationWithRelations[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getWorkspaceSprintsWithStats(
    workspaceId: string
  ...
  async updateSprint(
    ...
    const updateData: Partial<
      InsertSprint & { startDate?: Date; endDate?: Date }
    > = {
      ...updates,
  ...
  async getSprintTasks(sprintId: string): Promise<TaskWithRelations[]> {
    ...
    for (const task of taskList) {
      ...
      const assignee = task.assignedTo
        ? await this.getUser(task.assignedTo)
        : undefined;
  ...
  async getPendingWorkspaceInvitations(
    workspaceId: string
  ): Promise<WorkspaceInvitationWithRelations[]> {
    const results = await db
      .select()
